#!/bin/bash
# Script para verificar después de un deploy

SERVICE_NAME="checkin24hs_dashboard"
DOMAIN="dashboard.checkin24hs.com"

echo "=========================================="
echo "VERIFICAR DESPUES DE DEPLOY"
echo "=========================================="
echo ""
...
echo "=== 4. Verificar Display de Versión ==="
...
echo "=== 5. Verificar JavaScript de versión ==="
...
echo "=== 7. Verificar labels de Traefik ==="
...
if [ "$TRAEFIK_LABELS" -gt 0 ]; then
    echo "✅ OK: Labels de Traefik encontradas ($TRAEFIK_LABELS labels)"
    docker service inspect "$SERVICE_NAME" --format '{{range $key, $value := .Spec.Labels}}{{$key}}={{$value}}{"\n"}}{{end}}' | grep traefik | sort | head -5
else
...
if [ "$BUILD_NUMBER" = "5" ] && docker exec "$CONTAINER" grep -q "version-display" /app/dashboard.html; then
    ...
    echo ""
    echo "El contenedor no tiene la versión correcta."
    echo "Verifica que EasyPanel esté usando el commit más reciente."
fi
echo "=========================================="
echo ""
