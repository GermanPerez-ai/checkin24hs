...
function extractName(text, subject) {
  const nombre = field(text, '(?:Nombre|Hu[eé]sped|Titular|Pasajero|Cliente)');
  if (nombre && nombre.length >= 2 && nombre.length <= 80) return nombre.split(/\s{2,}|,/)[0].trim();
  const s = String(subject || '').replace(/^(RE|RV|FW|Fwd)\s*:\s*/i, '').trim();
  const m = s.match(/^Confirmaci[oó]n(?: de reserva)?(?:\s*[-–:]\s*|\s+)(.+)$/i);
  if (m) {
    const name = m[1].replace(/\bcorralco\b/gi, '').trim().replace(/^[-–:\s]+|[-–:\s]+$/g, '');
    if (name && !/consulta|tarifas|fotograf/i.test(name)) return name;
  }
  return null;
}
...
function parseCorralcoConfirmation(mail) {
  ...
  const nombre = extractName(text, subject);
