...
const FlorLearningSystem = {
    ...
    saveInteraction(interaction) {
        ...
        try {
            ...
            // Guardar también en Supabase (Flor usa todo desde Supabase)
            if (typeof window !== 'undefined' && window.supabaseClient && typeof window.supabaseClient.isInitialized === 'function' && window.supabaseClient.isInitialized()) {
                try {
                    window.supabaseClient.client.from('flor_interactions').insert({
                        id: enrichedInteraction.id,
                        phone: 'web',
                        user_message: enrichedInteraction.userMessage,
                        bot_response: enrichedInteraction.botResponse,
                        intent: enrichedInteraction.intent || null,
                        hotel_id: enrichedInteraction.hotelId || null,
                        success: enrichedInteraction.success,
                        used_ai: enrichedInteraction.usedAI || false
                    }).then(({ error }) => {
                        if (error) console.warn('[Flor Learning] Supabase insert:', error.message);
                    });
                } catch (e) { console.warn('[Flor Learning] Supabase:', e.message); }
            }
