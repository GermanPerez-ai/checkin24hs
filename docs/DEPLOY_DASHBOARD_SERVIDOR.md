...
3. **(Opcional)** Si querés una URL tipo `/v82/` para evitar caché del navegador:
   - En **`deploy/dashboard-html/server.js`** agregá el mismo patrón que para `/v81/` y `/v82/` pero para el nuevo número (ej. `/v82/` ya está; para 83 agregar `else if (urlPath.startsWith('/v83'))` y el replace).
...
- En el servidor:
  ...
  ```
  Deberían devolver el número de build que pusiste (ej. 82 si actualizaste a 82).
