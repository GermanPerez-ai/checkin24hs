# Script de Backup Exclusivo en Google Drive para Checkin24hs
# Versión mejorada y fácil de usar
...
function New-CompressedBackup {
    ...
    try {
        ...
        if (!(Test-Path $SourcePath)) {
            Write-Host "Error: El directorio fuente no existe: $SourcePath"
            return $null
        }
        ...
        # Crear el archivo ZIP usando .NET para evitar problemas de coordenadas
...
function Show-Status {
    Write-Host "=== ESTADO DEL BACKUP EN GOOGLE DRIVE ==="
    Write-Host ""
    
    # Verificar Google Drive
    if ($isGoogleDriveInstalled) {
        Write-Host "Google Drive: INSTALADO"
        Write-Host "Ruta: $googleDrivePath"
    } else {
        Write-Host "Google Drive: NO INSTALADO"
    }
    ...
    $backupPath = ".\backups"
    $localBackups = Get-ChildItem -Path $backupPath -Directory | Sort-Object CreationTime -Descending
    Write-Host ""
    Write-Host "Backups locales disponibles:"
    if ($localBackups.Count -gt 0) {
        $localBackups | Select-Object -First 5 | ForEach-Object {
        ...
        Write-Host "- No hay backups locales"
    }
    
    # Verificar backups en Google Drive
    $googleBackupPath = Join-Path $googleDrivePath "Checkin24hs_Backups"
    if (Test-Path $googleBackupPath) {
        $googleBackups = Get-ChildItem -Path $googleBackupPath -File | Sort-Object CreationTime -Descending
        Write-Host ""
        Write-Host "Backups en Google Drive:"
        if ($googleBackups.Count -gt 0) {
            $googleBackups | Select-Object -First 5 | ForEach-Object {
                $size = $_.Length
                Write-Host "- $($_.Name) ($([math]::Round($size / 1MB, 2)) MB)"
            }
        } else {
            Write-Host "- No hay backups en Google Drive"
        }
    } else {
        Write-Host ""
        Write-Host "Backups en Google Drive:"
        Write-Host "- Carpeta no creada aún"
    }
}
...
function Initialize-AutoBackup {
    ...
    $scriptPath = Join-Path (Get-Location) "backup_google_drive.ps1"
...
if ($Test) {
    ...
    Write-Host "2. Ver estado actual (. -Status)"
    Write-Host "3. Ejecutar backup manual (. -Manual)"
    Write-Host "4. Configurar backup automático (. -Setup)"
    Write-Host ""
