...
class SupabaseClient {
    ...
    async createHotel(hotel) {
        ...
        try {
            ...
            if (Array.isArray(payload.images) && payload.images.length > 0) {
                payload.imagen_principal = payload.images[0] || null;
                payload.galeria_fotos = payload.images.length > 1 ? payload.images.slice(1) : [];
            }
            if (payload.name && !payload.slug) {
                payload.slug = (payload.name || '').toLowerCase()
                    .replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
            }
            if (payload.rating != null) payload.puntuacion_num = Number(payload.rating);
            if (payload.price != null) payload.precio_desde = Number(payload.price);
    ...
    async updateHotel(id, updates) {
        ...
        try {
            ...
            // Rellenar columnas que usa la web pública (www.checkin24hs.com) para que refleje los cambios del dashboard
            if (Array.isArray(cleanUpdates.images) && cleanUpdates.images.length > 0) {
                cleanUpdates.imagen_principal = cleanUpdates.images[0] || null;
                cleanUpdates.galeria_fotos = cleanUpdates.images.length > 1 ? cleanUpdates.images.slice(1) : [];
            }
            if (cleanUpdates.name && !cleanUpdates.slug) {
                cleanUpdates.slug = (cleanUpdates.name || '').toLowerCase()
                    .replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
            }
            if (cleanUpdates.rating != null) {
                cleanUpdates.puntuacion_num = Number(cleanUpdates.rating);
            }
            if (cleanUpdates.price != null) {
                cleanUpdates.precio_desde = Number(cleanUpdates.price);
            }
    ...
    async getWhatsAppMessages(chatId, limit = 100) {
        ...
        try {
            // Pedir los N más RECIENTES (desc), luego invertir para mostrar cronológico
            const { data, error } = await this.client
            ...
            const list = data || [];
            return list.length > 0 ? list.reverse() : list;
        } catch (error) {
